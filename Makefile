...
SEED_FLAGS := $(if $(users),--users $(users)) $(if $(models),--models $(models)) $(if $(domain),--domain $(domain)) $(if $(passwd),--passwd '$(passwd)')

.PHONY: seed
seed: ## Load the demo dataset: make seed [users=5] [models=20] [domain=b.example] [passwd=...]
	@# The command prints the one password every seeded user logs in with.
	docker compose exec -T app go run ./cmd/pilam seed $(SEED_FLAGS)
